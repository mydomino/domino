class AnalyticsController < ApplicationController
  ...
  def show
    ...
    if(@filter == 'all')
      @recent_events = Recommendation.done.includes(:recommendable, :concierge).timestamped.order(:updated_at).last(50)
    else
      @filter = 'mine'
      @recent_events = Recommendation.done.includes(:recommendable, :concierge).timestamped.where(concierge_id: current_concierge.id).order(:updated_at).last(50)
    end
